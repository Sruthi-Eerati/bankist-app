...
const formatMovementDate = (date, locale) => {
  const days = (date1, date2) => Math.round(Math.abs(date2 - date1) / (1000 * 60 * 60 * 24));
...
const calcDisplayBalance = function (acc) {
  ...
  labelBalance.textContent = formattedCurr(acc.balance, acc.locale, acc.currency);
};
...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (
    ...
    alert('Check the balance to tranfer');
  }
});
